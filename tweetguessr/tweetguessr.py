...
class TweetGuessr:
    ...
    def __init__(self, args):
        self.args = args
        self.util = Util(args['face_recognition'])

    def classify_tweets(self, face_recognition=False, min_confidence=0.75):
        ...
        with open(TWEETS_PATH) as file:
            ...
            for tweet in content:
                ...
                if face_recognition and res['gender'] == 'unknown':
                    ...
                    self._perform_face_recognition(res, tweet_json)
    ...
    def generate_sets(self, male_tweets_dict, female_tweets_dict, percentage_test):
        """
        ...
        male_test, male_training = self._get_test_training_sets(male_tweets_dict, test_male_len)
        ...
        female_test, female_training = self._get_test_training_sets(female_tweets_dict, test_female_len)
    ...
    def build_lexicon(self, male_training, female_training, llr_threshold=0.5):
        """
        ...
        male_words_freq, male_word_count = self._get_word_freq(male_training)
        # print(sorted(male_words_freq.items(), key=operator.itemgetter(1), reverse=True))
        #print(male_word_count)
        female_words_freq, female_word_count = self._get_word_freq(female_training)
        ...
        for word in all_words:
            ...
            lexicon[word] = self.util.root_log_likelihood_ratio(male_words_freq[word], female_words_freq[word],
                                                      male_word_count, female_word_count)
        return self._trim_lexicon(lexicon, llr_threshold)

    def _trim_lexicon(self, lexicon, threshold):
        """
    ...
    def _get_word_freq(self, training_set):
        """
        ...
        for user_tweets in training_set.values():
            for tweet in user_tweets:
                for word in self.util.normalise(tweet).split():  # normalise tweet text
                    words_freq[word] += 1
                    word_count += 1
        return words_freq, word_count

    def _print_sets_size_feedback(self, training_set, test_set, num_users, test_len, gender):
        """
    ...
    def _get_test_training_sets(self, full_user_tweets_dict, test_len):
        test_set = dict()
    ...
    def _perform_face_recognition(self, res, tweet_json):
        """
    ...
    def _get_and_store_num_tweets(self, male_full_user_tweets_dict, female_full_user_tweets_dict):
        """
        ...
        """
        male_user_num_tweets = self._get_user_num_tweets(male_full_user_tweets_dict, gender='male')
        # sorted_male_user_num_tweets = _write_file_user_num_tweets(male_user_num_tweets, gender='male')

        female_user_num_tweets = self._get_user_num_tweets(female_full_user_tweets_dict, gender='female')
    ...
    def _get_user_num_tweets(self, full_user_tweets_dict, gender):
        """
    ...
    def _write_file_user_num_tweets(self, user_num_tweets, gender):
        """
    ...
    def perform_tests(self, lexicon, male_test, female_test):
        print('Performing tests...')
        recall_males, precision_males = self.perform_test(lexicon, male_test, 'male')
        recall_females, precision_females = self.perform_test(lexicon, female_test, 'female')
        ...
        if self.args['write_results']:
            ...
            with open(current_path + '/data/result_tests.tsv', 'a') as f:
                f.write("# time:{} args:{}\n".format(datetime.datetime.now(), self.ordered_args(self.args)))
                f.write("m_rec\t{}\nm_pre\t{}\n".format(recall_males, precision_males))
                f.write("f_rec\t{}\nf_pre\t{}\n\n".format(recall_females, precision_females))

    def perform_test(self, lexicon, test_set, gender):
        """
    ...
    def main(self):
        ...
        male_tweets_dict, female_tweets_dict = self.classify_tweets(self.args['face_recognition'], self.args['min_conf'])
        sets = self.generate_sets(male_tweets_dict, female_tweets_dict, percentage_test=0.2)
